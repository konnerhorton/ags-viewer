...
with st.sidebar:
    ...
    if crs_from:
        st.session_state.crs_from = crs_from
...
try:
    transformer = Transformer.from_crs(
        st.session_state.crs_from, crs_to, always_xy=True
    )
except:
    st.write("Please input a valid EDPS")

if df is not None and not df.empty and transformer and st.session_state.crs_from:
